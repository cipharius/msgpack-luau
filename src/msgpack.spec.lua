return function()
  ...
  describe("encode", function()
    ...
    it("can encode positive integers", function()
      expect(msgpack.encode(0)).to.equal("\x00")
      expect(msgpack.encode(1)).to.equal("\x01")
      expect(msgpack.encode(127)).to.equal("\x7F")
      expect(msgpack.encode(128)).to.equal("\xCC\x80")
      expect(msgpack.encode(255)).to.equal("\xCC\xFF")
      expect(msgpack.encode(256)).to.equal("\xCD\x01\x00")
      expect(msgpack.encode(65535)).to.equal("\xCD\xFF\xFF")
      expect(msgpack.encode(65536)).to.equal("\xCE\x00\x01\x00\x00")
      expect(msgpack.encode(4294967295)).to.equal("\xCE\xFF\xFF\xFF\xFF")
    end)

    it("can encode negative integers", function()
      expect(msgpack.encode(-1)).to.equal("\xFF")
      expect(msgpack.encode(-32)).to.equal("\xE0")
      expect(msgpack.encode(-33)).to.equal("\xD0\xDF")
      expect(msgpack.encode(-128)).to.equal("\xD0\x80")
      expect(msgpack.encode(-129)).to.equal("\xD1\xFF\x7F")
